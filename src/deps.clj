...
;; Compute orga deps similarity

(defn mean [a] (/ (reduce + a) (count a)))

(defn standard-population-deviation [a]
  (let [mn (mean a)]
    (Math/sqrt
     (/ (reduce #(+ %1 (Math/pow (- %2 mn) 2)) 0 a)
        ;; (dec (count a)) ;; For sample deviation
        (count a)))))

(defn normalized-euclidian-distance [a1 a2 mean dev]
  (matrix/distance ;; Euclidian distance
   (map #(if (zero? %3) 100000 (/ (- %1 %2) %3)) a1 mean dev)
   (map #(if (zero? %3) 100000 (/ (- %1 %2) %3)) a2 mean dev)))

(defn repos-deps-matrix [deps repos-by-orgas]
  (->>
   (map
    (fn [[k v]] ; k is :organisation_nom, v is the list of repos with :deps
      (hash-map
       k
       (map
        #(reduce + (map :c %))
    ...
    repos-by-orgas)
   (apply merge)
   ;; Only consider orga with more than 20 repos?
   ;; (filter #(> (reduce + (val %)) 100))
   ))

(defn repos-deps-sim [repos-deps-matrix dep-means dep-dev]
  (map (fn [[orga deps-array]]
         (hash-map
          orga
          (take
           5 ;; Take 5 similar organisations
           (filter
            #(not (= % orga))
            (keys
             (sort-by
              ...
              (into
               {}
               (map
                (fn [[orga-in deps-array-in]]
                  (hash-map
                   orga-in
                   (normalized-euclidian-distance
                    deps-array deps-array-in dep-means dep-dev)))
                repos-deps-matrix))))))))
       repos-deps-matrix))

(defn repos-with-deps [repos]
  (map #(select-keys % [:organisation_nom
                        :repertoire_url
                        :deps])
       (filter #(seq (remove nil? (:deps %))) repos)))

(defn deps-short [deps]
  (map #(select-keys % [:n :t]) deps))

(defn spit-deps-orgas-similarity [repos deps-all]
  (let [deps0           (deps-short deps-all)
        repos-with-deps (repos-with-deps repos)
        repos-by-orgas  (group-by :organisation_nom repos-with-deps)
        res             (repos-deps-matrix deps0 repos-by-orgas)
        dep-vals        (matrix/transpose (vals res))
        dep-means       (map mean dep-vals)
        dep-dev         (map standard-population-deviation dep-vals)
        res2            (repos-deps-sim res dep-means dep-dev)]
    ;; (println (pr-str (take 5 res)))
    (spit "deps-orgas-sim.json"
          (json/write-value-as-string
           (into {} (filter #(seq (val %)) (into {} (remove nil? res2))))))
    (println "Updated deps-orgas-sim.json")))
